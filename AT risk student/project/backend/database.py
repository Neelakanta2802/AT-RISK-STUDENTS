"""
Database connection and utilities for Supabase or standard PostgreSQL (Neon) integration.
"""
import logging
import json
import os
...
# ============================================================================
# PostgreSQL / Neon Compatibility Layer
# ============================================================================

class PostgresConnectionPool:
    def __init__(self, connection_string: str):
        self.conn_str = connection_string
        # Render/Neon standard compatibility
        if self.conn_str.startswith("postgres://"):
            self.conn_str = "postgresql://" + self.conn_str[len("postgres://"):]
        
        try:
            from psycopg2.pool import ThreadedConnectionPool
            # Initialize connection pool with minimum 2 and maximum 20 connections
            self.pool = ThreadedConnectionPool(2, 20, self.conn_str, cursor_factory=RealDictCursor)
            logger.info("✅ psycopg2 ThreadedConnectionPool initialized successfully")
        except Exception as e:
            logger.error(f"❌ Failed to initialize ThreadedConnectionPool: {e}", exc_info=True)
            self.pool = None
    ...
    def get_connection(self):
        if self.pool:
            try:
                ...
                if conn.closed:
                    logger.warning("Connection from pool is closed. Reconnecting...")
                    conn = psycopg2.connect(self.conn_str, cursor_factory=RealDictCursor)
                conn.autocommit = True
                return conn
            except Exception as e:
                logger.error(f"Error getting connection from pool: {e}. Falling back to direct connection.")
...
class PostgresResponse:
    def __init__(self, data: List[Dict[str, Any]], error: Any = None):
        self.data = data
        self.error = error


class PostgresQueryBuilder:
    def __init__(self, table_name: str, pool: PostgresConnectionPool):
        ...
        self.filters = []  # tuples of (column, operator, value)
        self.order_by = None
        self.limit_val = None
    ...
    def insert(self, data: Any, returning: str = None):
        self.op = "insert"
    ...
    def execute(self) -> PostgresResponse:
        ...
        try:
            ...
            if self.op == "select":
                ...
                where_clause, filter_params = self._build_where()
                if where_clause:
                    ...
                    for row in self.insert_data:
                        ...
                        for col in columns:
                            ...
                            if isinstance(val, (dict, list)):
                                val = json.dumps(val)
                            params.append(val)
                        val_placeholders.append(f"({', '.join(placeholders)})")
                    query = f"INSERT INTO {self.table_name} ({col_str}) VALUES {', '.join(val_placeholders)} RETURNING *"
                else:
                    columns = list(self.insert_data.keys())
                    col_str = ", ".join(columns)
                    placeholders = ", ".join(["%s"] * len(columns))
                    for col in columns:
                        val = self.insert_data.get(col)
                        if isinstance(val, (dict, list)):
                            val = json.dumps(val)
                        params.append(val)
                    query = f"INSERT INTO {self.table_name} ({col_str}) VALUES ({placeholders}) RETURNING *"

            elif self.op == "update":
                set_clauses = []
                for col in self.update_data.keys():
                    set_clauses.append(f"{col} = %s")
                    val = self.update_data.get(col)
                    if isinstance(val, (dict, list)):
                        val = json.dumps(val)
                    params.append(val)
            ...
            logger.debug(f"Neon Executing: {query} with params {params}")
            cur.execute(query, params)

            data = []
            if self.op in ["select", "delete", "insert", "update"]:
                try:
                    rows = cur.fetchall()
                    data = [dict(r) for r in rows]
                except psycopg2.ProgrammingError:
                    pass # Query returns no rows (e.g. DELETE/UPDATE without returning or no match)

            return PostgresResponse(data)

        except Exception as e:
            logger.error(f"Neon Database Query Error: {e}\nQuery: {query}\nParams: {params}", exc_info=True)
            return PostgresResponse([], error=e)
        finally:
            if cur:
                cur.close()
            if conn:
...
class PostgresPostgrestClient:
    def __init__(self, pool: PostgresConnectionPool):
        self.pool = pool

    def table(self, table_name: str):
        return PostgresQueryBuilder(table_name, self.pool)
...
class Database:
    """Database wrapper for Supabase or direct PostgreSQL/Neon operations."""
    ...
    def __init__(self):
        """Initialize database client (Postgres/Neon or Supabase)."""
        try:
            # 1. Check if direct PostgreSQL / Neon URL is provided
            db_url = settings.database_url or os.environ.get("DATABASE_URL")
            if db_url:
                logger.info("Initializing direct PostgreSQL / Neon connection pool...")
                try:
                    pool = PostgresConnectionPool(db_url)
                    self.client = PostgresPostgrestClient(pool)
                    
                    # Test query to verify Neon connection
                    test_response = self.client.table('students').select('id').limit(1).execute()
                    logger.info("✅ Direct PostgreSQL / Neon connection established and verified")
                    return
                except Exception as pg_err:
                    logger.error(f"❌ Failed to connect to Neon/Postgres database: {pg_err}", exc_info=True)
                    logger.error("Will attempt to fall back to Supabase client if credentials exist...")

            # 2. Fallback to Supabase client
            logger.info(f"DB Init - URL: '{settings.supabase_url[:10] if settings.supabase_url else 'N/A'}...', Key length: {len(settings.supabase_key) if settings.supabase_key else 0}")
            if not settings.supabase_url or not settings.supabase_key:
                logger.error("Supabase credentials missing and no DATABASE_URL provided!")
                self.client = None
                return
