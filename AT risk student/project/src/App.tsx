...
function AppContent() {
  ...
  // Listen for hash changes to navigate directly to student profile
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash;
      if (hash.startsWith('#student-')) {
        const studentId = hash.substring('#student-'.length);
        if (studentId) {
          console.log(`🔗 Hash change: navigating to student profile ${studentId}`);
    ...
    // Run once on load to handle initial deep link
    handleHashChange();
